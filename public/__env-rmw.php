<?php
/**
 * Boels CORE — Eenmalig: rmw.sorai.nl toevoegen aan
...
echo "Boels CORE — rmw.sorai.nl toevoegen aan Sanctum\n" . str_repeat('=', 50) . "\n\n";

if (preg_match('/^SANCTUM_STATEFUL_DOMAINS=(.*)$/m', $contents, $sm)) {
    $domains = array_filter(array_map('trim', explode(',', $sm[1])));
    if (in_array('rmw.sorai.nl', $domains)) {
        echo "rmw.sorai.nl stond er al in.\n";
    } else {
        $domains[] = 'rmw.sorai.nl';
