<?php
/**
 * Boels CORE — Eenmalig: projectschade.sorai.nl toevoegen aan
...
echo "Boels CORE — projectschade.sorai.nl toevoegen aan Sanctum\n" . str_repeat('=', 50) . "\n\n";

if (preg_match('/^SANCTUM_STATEFUL_DOMAINS=(.*)$/m', $contents, $sm)) {
    $domains = array_filter(array_map('trim', explode(',', $sm[1])));
    if (in_array('projectschade.sorai.nl', $domains)) {
        echo "projectschade.sorai.nl stond er al in.\n";
    } else {
        $domains[] = 'projectschade.sorai.nl';
