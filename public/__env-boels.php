<?php
/**
 * Boels CORE — Eenmalig: boels.sorai.nl (inhuur-app) toevoegen aan
...
echo "Boels CORE — boels.sorai.nl toevoegen aan Sanctum\n" . str_repeat('=', 50) . "\n\n";

if (preg_match('/^SANCTUM_STATEFUL_DOMAINS=(.*)$/m', $contents, $sm)) {
    $domains = array_filter(array_map('trim', explode(',', $sm[1])));
    if (in_array('boels.sorai.nl', $domains)) {
        echo "boels.sorai.nl stond er al in.\n";
    } else {
        $domains[] = 'boels.sorai.nl';
