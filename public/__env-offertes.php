<?php
/**
 * Boels CORE — Eenmalig: offertes.sorai.nl toevoegen aan SANCTUM_STATEFUL_DOMAINS.
...
echo "Boels CORE — offertes.sorai.nl toevoegen aan Sanctum\n" . str_repeat('=', 50) . "\n\n";

if (preg_match('/^SANCTUM_STATEFUL_DOMAINS=(.*)$/m', $contents, $sm)) {
    $domains = array_filter(array_map('trim', explode(',', $sm[1])));
    if (in_array('offertes.sorai.nl', $domains)) {
        echo "offertes.sorai.nl stond er al in.\n";
    } else {
        $domains[] = 'offertes.sorai.nl';
