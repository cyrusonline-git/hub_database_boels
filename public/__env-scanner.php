<?php
/**
 * Boels CORE — Eenmalig: scanner.sorai.nl toevoegen aan SANCTUM_STATEFUL_DOMAINS.
 * Nodig zodat de Scanner App (kale PHP) via cookie-relay /api/me kan gebruiken.
 *
 * Sleutel = DEPLOY_SECRET uit de server-.env.
...
if (! hash_equals(trim($m[1]), (string) ($_GET['k'] ?? ''))) {
    http_response_code(403);
    exit('forbidden');
}

header('Content-Type: text/plain; charset=utf-8');
echo "Boels CORE — scanner.sorai.nl toevoegen aan Sanctum\n" . str_repeat('=', 50) . "\n\n";

if (preg_match('/^SANCTUM_STATEFUL_DOMAINS=(.*)$/m', $contents, $sm)) {
    $domains = array_filter(array_map('trim', explode(',', $sm[1])));
    if (in_array('scanner.sorai.nl', $domains)) {
        echo "scanner.sorai.nl stond er al in.\n";
    } else {
        $domains[] = 'scanner.sorai.nl';
